...
def receive_params():
    ...
    # Create SAS connection
    conn = saspy.SASsession(cfgfile="/root/sascfg_personal.py")

    dt = request.json

    # Create library to hold info
    conn.submitLOG("""     
    ...
    person = [{
                ...
                "DOB": dt["DOB"],
                "NEMCAID": dt["NEMCAID"],
                "LTIMCAID": dt["LTIMCAID"],
                "OREC": dt["OREC"]
            }]

    person_dt = pd.read_json(json.dumps(person))

    person_dt["DOB"] = pd.to_datetime(person_dt["DOB"])
    ...
    diag_dt = pd.read_json(json.dumps(diag))

    # Create sas7bdat and save to library on step 1
    conn.df2sd(person_dt, table = "PERSON", libref = "IN1", datetimes = {"DOB":"date"}, outfmts={"DOB" : "date9."})
    conn.df2sd(diag_dt, table = "DIAG", libref = "IN1")

    # Run Code
    run_sas_code(conn)

    # Return Results to Client
    final_data = conn.sasdata2dataframe(table = "PERSON", libref = "OUT")

    cols_keep = ["SCORE_COMMUNITY_NA", "SCORE_COMMUNITY_ND", "SCORE_COMMUNITY_FBA", "SCORE_COMMUNITY_FBD", "SCORE_COMMUNITY_PBA", "SCORE_COMMUNITY_PBD", "SCORE_INSTITUTIONAL", "SCORE_NEW_ENROLLEE", "SCORE_SNP_NEW_ENROLLEE"]

    final_data = final_data[cols_keep]

    return(final_data.to_json())

# Execute SAS CMS Code
def run_sas_code(conn):

    results_dict = conn.submitLOG("""FILENAME inc  "/data/CMSHCC/transport/C2419P1M";
